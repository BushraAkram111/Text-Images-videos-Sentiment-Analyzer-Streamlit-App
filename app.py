...
def print_result_header():
    ...
    components.html(
        ...
        "What do you think about our results?</p>",
        height=150,
    )
...
def show_image_page():
    ...
    st.image(
        "https://logicpursuits.com/wp-content/uploads/2022/07/Why-Captemo-Emotion-Sentiment-Analyzer.jpg",
        use_column_width=True,
    )

    st.text(
        """In the Image Analyzer section, you can upload an image to analyze the emotions 
...
def analyze_image(image):
    ...
    detector = FER(mtcnn=True, cascade_file="haarcascade_frontalface_default.xml")
    ...
    if emotion_results:
        for result in emotion_results:
            ...
            img_array = cv2.putText(
                img_array,
                f"{max_emotion} {getEmoji[max_emotion]}",
...
def show_video_page():
    ...
    st.image(
        "https://cdn.analyticsvidhya.com/wp-content/uploads/2021/06/Learn-How-to-Implement-Face-Recognition-using-OpenCV-with-Python-80.jpg",
        use_column_width=True,
    )

    st.text(
        """Upload your video and let's find sentiments in it. The Video Analyzer section 
...
def analyze_video(video_path):
    ...
    if emotions_list:
        ...
        fig.update_layout(
            title="Average Emotions in Video",
            xaxis_title="Emotion",
...
def render_text_analysis_page():
    ...
    st.image(
        "https://miro.medium.com/v2/resize:fit:1358/0*V00v1_1CWWcQ3G6G",
        use_column_width=True,
    )

    st.text(
        """In the Text Analysis section, you can input text to analyze its sentiment using 
...
def get_sentiments(user_text, analysis_type):
    if analysis_type == "Positive/Negative/Neutral - TextBlob":
        ...
        fig.update_layout(
            title="Sentiment Analysis", xaxis_title="Sentiment", yaxis_title="Count"
        )
        st.plotly_chart(fig)
        ...
        fig.update_layout(
            title="Emotion Analysis", xaxis_title="Emotion", yaxis_title="Score"
        )
        st.plotly_chart(fig)
